...
/// 对于确定支持编辑的属性和实例变量为 YES，对于所有方法为 NO。
@property (nonatomic, readonly) BOOL isEditable;
/// 对于实例变量为 NO，对于支持的方法和属性为 YES
...
/// 对于方法，是方法调用屏幕。对于其他所有情况，是对象浏览器。
- (UIViewController *)viewerWithTarget:(id)object;
/// 对于方法和协议，为 nil。对于其他所有情况，为字段编辑器屏幕。
/// 任何更改提交后，给定的部分都会重新加载。
...
/// 一个数组，其中每 2 个元素组成一个键值对。键是
/// 要复制的内容的描述，例如“名称”，值是将要复制的内容。
- (NSArray<NSString *> *)copiableMetadataWithTarget:(id)object;
/// 属性和实例变量如果持有对象，则返回该对象的地址。
- (NSString *)contextualSubtitleWithTarget:(id)object;

@end

// 即使属性是只读的，它仍然可能通过 setter 进行编辑。
// 检查 isEditable 不会反映这一点，除非该属性是用类初始化的。
